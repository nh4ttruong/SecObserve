...
# Generous: the lock is only held for the writes of one product, branch and service
LOCK_TIMEOUT = "30s"
...
def _set_lock_timeout() -> None:
    ...
    PostgreSQL waits for a row lock forever by default, so a recalculation whose product row
    is held by a long running transaction would block one of the few Huey workers until the
    process is restarted. A timeout turns that into a failed task, which is retried by the
    next import of the same scope. MySQL needs nothing here, innodb_lock_wait_timeout is
